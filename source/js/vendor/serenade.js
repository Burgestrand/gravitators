...
return new Parser;
})();var AssociationCollection, COMMENT, Cache, Collection, Compile, DynamicNode, Event, IDENTIFIER, KEYWORDS, LITERAL, Lexer, MULTI_DENT, Map, Model, Node, Property, PropertyAccessor, PropertyDefinition, STRING, Serenade, Transform, View, WHITESPACE, assignUnlessEqual, bindToProperty, capitalize, compile, def, defineEvent, defineOptions, defineProperty, extend, format, formatTextValue, formatValue, hash, hash_current, hash_max, hash_prefix, idCounter, isArray, merge, nextTick, nextTickList, nextTickTimeout, normalize, pairToObject, primitiveTypes, safeDelete, safePush, serializeObject, settings,
...
PropertyAccessor = (function() {
  ...
  PropertyAccessor.prototype.format = function(val) {
...
Node = (function() {
  ...
  Node.prototype.addBoundClass = function(className) {
    var _ref;
    this.boundClasses || (this.boundClasses = new Collection());
    if (!((_ref = this.boundClasses) != null ? _ref.includes(className) : void 0)) {
      this.boundClasses.push(className);
      return this.updateClass();
    }
  };

  Node.prototype.removeBoundClass = function(className) {
    var index;
    if (this.boundClasses) {
      index = this.boundClasses.indexOf(className);
      this.boundClasses["delete"](className);
      return this.updateClass();
    }
  };

  Node.prototype.setAttributeClasses = function(name, value) {
    if (name == null) {
      name = "__self__";
    }
    this.attributeClasses || (this.attributeClasses = {});
    this.attributeClasses[name] = value;
    return this.updateClass();
  };
  ...
  Node.prototype.updateClass = function() {
    var classes, value, _, _ref, _ref1;
    classes = this.ast.classes;
    if (this.attributeClasses) {
      _ref = this.attributeClasses;
      for (_ in _ref) {
    ...
    if ((_ref1 = this.boundClasses) != null ? _ref1.length : void 0) {
...
formatValue = function(ast, model, value) {
  var formatter;
...
bindToProperty = function(node, model, name, cb) {
  var value, _ref;
  value = model[name];
  if ((_ref = model["" + name + "_property"]) != null) {
    if (typeof _ref.registerGlobal === "function") {
      _ref.registerGlobal(value);
    }
  }
...
Property = {
  style: function(ast, node, model, controller) {
    ...
    if (ast.bound && ast.value) {
      return bindToProperty(node, model, ast.value, function(_, value) {
        return assignUnlessEqual(node.element.style, ast.name, formatValue(ast, model, value));
      });
    } else {
      return node.element.style[ast.name] = (_ref = ast.value) != null ? _ref : model;
    }
  },
  ...
  "class": function(ast, node, model, controller) {
    return bindToProperty(node, model, ast.value, function(_, value) {
      if (value) {
        return node.addBoundClass(ast.name);
      } else {
        return node.removeBoundClass(ast.name);
      }
    });
  },
  binding: function(ast, node, model, controller) {
    var domUpdated, element, handler, _ref;
    ...
    bindToProperty(node, model, ast.value, function(_, value) {
  ...
  attribute: function(ast, node, model, controller) {
    ...
    set = function(value) {
      if (ast.name === 'value') {
        ...
        if (node.element.hasAttribute(ast.name)) {
          return node.element.removeAttribute(ast.name);
        }
      } else {
        if (value === 0) {
          value = "0";
        }
        if (node.element.getAttribute(ast.name) !== value) {
          return node.element.setAttribute(ast.name, value);
        }
      }
    };
    if (ast.bound && ast.value) {
      return bindToProperty(node, model, ast.value, function(_, value) {
        return set(formatValue(ast, model, value));
      });
    } else {
      return set((_ref = ast.value) != null ? _ref : model);
    }
  },
  ...
  property: function(ast, node, model, controller) {
    var set, _ref;
    set = function(value) {
      return assignUnlessEqual(node.element, ast.name, value);
    };
    if (ast.bound && ast.value) {
      return bindToProperty(node, model, ast.value, function(_, value) {
        return set(value);
...
Compile = {
  ...
  helper: function(ast, model, controller) {
    ...
    renderBlock = function(model, blockController) {
      if (model == null) {
        model = model;
      }
      ...
      return new View(null, ast.children).render(model, blockController, controller, controller === blockController);
    };
  ...
  text: function(ast, model, controller) {
    var node, textNode, _ref;
    if (ast.bound && ast.value) {
      textNode = Serenade.document.createTextNode("");
      node = new Node(ast, textNode);
      bindToProperty(node, model, ast.value, function(_, value) {
        return assignUnlessEqual(textNode, "nodeValue", formatTextValue(formatValue(ast, model, value)));
      });
      return node;
    } else {
      return new Node(ast, Serenade.document.createTextNode((_ref = ast.value) != null ? _ref : model));
    }
  },
  ...
  bound: function(ast, model, controller, callback) {
    var dynamic;
    dynamic = new DynamicNode(ast);
    bindToProperty(dynamic, model, ast.argument, function(before, after) {
      if (before !== after) {
